...
<ul class="<?php echo esc_attr(implode(' ', $classes)) ;?> clearfix">
    ...
    <li class="feature-post-item style-default hover-image-translate">
        ...
        <div class="content">
            ...
            <ul class="meta-feature fw-7 d-flex text-caption-2 mb_11">
                <li class="author">
                    <span class="by"><?php esc_html_e( 'by', 'zeng' ); ?></span>
                    <?php
        $author_id = get_post_field('post_author', get_the_ID());
        printf(
            ' <a class="link author-name" href="%s" title="%s" rel="author">%s</a>',
            esc_url( get_author_posts_url( $author_id ) ),
            esc_attr( sprintf( __( 'View all posts by %s', 'zeng' ), get_user_full_name($author_id) )),
...
<p>
    <label
        for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'themesflat' ); ?></label>
...
<p>
    <label
        for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select Category:', 'themesflat' ); ?></label>
    <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"
        name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>[]">
        <option value="" <?php selected( empty( $instance['category'] ) ); ?>>
            <?php esc_html_e( 'All', 'themesflat' ); ?></option>
        <?php               
...
<p>
    <label
        for="<?php echo esc_attr( $this->get_field_id( 'ids' ) ); ?>"><?php esc_html_e( 'Get Post by IDS EX:1,2,3', 'themesflat' ); ?></label>
...
<p>
    <label
        for="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>"><?php esc_html_e( 'Count:', 'themesflat' ); ?></label>
